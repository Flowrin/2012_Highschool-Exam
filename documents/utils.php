...
function return_file()
{
    ...
    if (ini_get('safe_mode')) {fpassthru($file);}
    else
    {
...
function random_html($id,$root,$page_cat)
{
  ...
  if(in_array($page_cat,$ita)) $rnd_key=array_search($page_cat,$ita);
  else $rnd_key=array_rand($ita);
  $res=xgfs($data,'<'.$ita[$rnd_key].'>','<END>');
...
function process_it()
{
 ...
 if (isset($_GET['action']))
 {
  ...
  elseif($version='version')echo $version;
 }
}
